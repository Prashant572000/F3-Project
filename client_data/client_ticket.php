...
if(isset($_POST['submit']))
{
    ...
    if($_FILES['file5']['name']) {
        $filename=explode("." , $_FILES['file5']['name']);
        if($filename[1]=='csv')  {
            $handle = fopen($_FILES['file5']['tmp_name'] , 'r');
            $c=0;
            $count=0;
            while($data=fgetcsv($handle)) {
                ...
                $checkexistdata="SELECT * FROM `client_ticket` Where `ORDER ID/SUB ORDER ID` = '$item2' AND `TICKET ID`='$item3'";
                $ressultcheckexistdata=mysqli_query($conn,$checkexistdata);
                $countexistdata=mysqli_num_rows($ressultcheckexistdata);
                if($countexistdata == 0) {
                        if($count > 1) {  
                $sql="INSERT INTO `client_ticket`(`DATE`, `ORDER ID/SUB ORDER ID`, `TICKET ID`, `PANEL`, `CLAIM TYPE`, `Status`, `Person`, `FOLLOW UP BY`) 
